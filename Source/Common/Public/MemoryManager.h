...
#define SGE_MEMORY_ALIGN( value, alignment ) ( ( ( value ) + ( (alignment) -1 ) ) & ~( (alignment) -1 ) )
	...
	*/
	...
	public:
		...
		so when the memory block is allocated,its header will be unuseful,to avoid waste,the memory of the header will
		also be a part of the memory which is allocated.There is a formula of memory size which need to be allocated:
		...
		*/
		...
		private:
			/*!
			@brief get the memory address of the next memory block by giving the memory address of the current memory
			block
