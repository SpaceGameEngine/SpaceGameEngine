...
namespace SpaceGameEngine
{
	...
	*/
	...
	inline void* (*QueryCopyAssignment())(void*, const void*)
	{
		if constexpr (std::is_copy_assignable_v<T>)
			return [](void* ptr, const void* arg) -> void* {
				SGE_ASSERT(NullPointerError, ptr);
				SGE_ASSERT(NullPointerError, arg);
				return reinterpret_cast<void*>(&(*reinterpret_cast<T*>(ptr) = *reinterpret_cast<const T*>(arg)));
			};
		else
			return nullptr;
	}
	...
	inline void* (*QueryMoveAssignment())(void*, void*)
	{
		if constexpr (std::is_move_assignable_v<T>)
			return [](void* ptr, void* arg) -> void* {
				SGE_ASSERT(NullPointerError, ptr);
				SGE_ASSERT(NullPointerError, arg);
				return reinterpret_cast<void*>(&(*reinterpret_cast<T*>(ptr) = std::move(*reinterpret_cast<T*>(arg))));
			};
		else
			return nullptr;
	}
	...
	{
		if constexpr (IsWeakEqualityComparable<T>)
	...
	*/
	...
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("The MetaData is invalid");
