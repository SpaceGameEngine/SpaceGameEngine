...
#error "this platform is not supported"
	...
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("Dll's name is empty.");
		static COMMON_API bool Judge(const String& name);
	};

	struct EmptyDllPathError
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("Dll's path is empty.");
		static COMMON_API bool Judge(const String& dll_path);
	};

	struct EmptySymbolError
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("Symbol's string is empty.");
		static COMMON_API bool Judge(const String& symbol);
	};

	struct LoadDllFailedError
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("Loading dll is failed.");
		static COMMON_API bool Judge(const DllHandle& re);
	};

	struct GetAddressFromDllFailedError
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("Getting address from dll is failed.");
		static COMMON_API bool Judge(void* re);
	};

	struct UnloadDllFailedError
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("Unloading dll is failed.");
		static COMMON_API bool Judge(int re);
	};

	struct NullDllHandleError
	{
		inline static const ErrorMessageChar pContent[] = SGE_ESTR("Dll's handle is null.");
