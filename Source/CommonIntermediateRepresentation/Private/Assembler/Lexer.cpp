...
Pair<Vector<CommonParser::Lexer::Token>, Vector<CommonParser::ParserError>> SpaceGameEngine::CommonIntermediateRepresentation::Assembler::GetTokens(const String& str)
{
	auto tokens_and_errors = CppLikeStyleLexer::GetTokens(str);

	return MakePair(RemoveToken(
						CombineToken(
							CombineToken(
								CombineToken(
									CombineToken(
										TransformToken(tokens_and_errors.m_First,
													   [=](const Token& token) {
