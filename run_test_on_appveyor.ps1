...
Get-ChildItem $solution_directory\Test\ | ForEach-Object{
	if($_.Attributes -eq "Directory")
	{
		...
		& $solution_directory\Binary\$test_name\$platform\$configuration\$test_name.exe -r junit -o $test_name.xml
		$wc.UploadFile("https://ci.appveyor.com/api/testresults/junit/$($env:APPVEYOR_JOB_ID)", (Resolve-Path .\$test_name.xml))
	}
}
